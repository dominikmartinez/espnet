...
class BeamSearchCTC(torch.nn.Module):
    ...
    def __init__(
        ...
        self.blank_penalty = np.log(blank_penalty)      # typically between 0.5 and 2.0
        self.sos = sos
        self.ctc = ctc
        self.lm = lm
    ...
    def ctc_search_step(
        ...
        for l in A_pruned:
            ...
            # Handle when prefix doesn't grow
            p_nb, p_b = p_prefix_next[l] # log0 , log0
            # Handle Blank - add to the total score
            p_b = np.logaddexp(p_b, p_ctc[blank] + p_prev_l + self.blank_penalty)
            # Handle Repeat - only add to the nb score
            p_nb = np.logaddexp(p_nb, p_ctc[l[-1]] + p_prefix[l][0] + self.blank_penalty)
            p_prefix_next[l] = (p_nb, p_b)
            A_next.add(l)

            for c in cs_pruned:
                if c == blank:
                    # Handled blank already
                    continue
                else:
                    l_plus = l + (int(c),)
                    p_nb, p_b = p_prefix_next[l_plus]
                    if c == l[-1]:
                        # Handled repeat already
                        continue
                    else:
                        # Handle non-repeat and non-blank
                        p_nb = np.logaddexp(p_nb, p_ctc[c] + p_prev_l)
                    if l_plus not in A_pruned and l_plus in p_prefix:
                        # Handle reviving pruned hypothesis
