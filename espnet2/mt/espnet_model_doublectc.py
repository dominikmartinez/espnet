...
class ESPnetMTModelDCTC(AbsESPnetModel):
    ...
    def __init__(
        ...
        self.mt_lego_encoder = mt_lego_encoder
        ...
        self.src_ctc_weight = src_ctc_weight
        if src_ctc_weight > 0:
            self.src_ctc = src_ctc
    ...
    def forward(
        ...
        **kwargs,
        ...
        stats = dict()
        if self.src_ctc_weight > 0:
            loss_src_ctc = self.src_ctc(encoder_out, encoder_out_lens, src_text, src_text_lengths)
            encoder_out, encoder_out_lens, _ = self.mt_lego_encoder(encoder_out, encoder_out_lens)
            # 3. Loss computation
            loss = self.src_ctc_weight * loss_src_ctc
            stats["loss_src_ctc"] = loss_src_ctc.detach()
        else:
        ...
        stats["loss_mt_att"]=loss_mt_att.detach()
        stats["acc"]=acc_mt_att
        stats["bleu"]=bleu_mt_att

        # 2b. CTC Branch
        if self.mt_ctc_weight > 0:
            loss_mt_ctc = self.mt_ctc(encoder_out, encoder_out_lens, text, text_lengths)
            # 3. Loss computation
            loss += (1-self.mt_ctc_weight - self.src_ctc_weight) * loss_mt_att + self.mt_ctc_weight * loss_mt_ctc
